...
async def test_gray_to_binary(dut: Graytobinary):
    ...
    assert len(set(results.values())) == NUM_VALUES, "Not all output values were unique."

    # Sort the outputs by their binary value
    sorted_results = [item[0] for item in sorted(results.items(), key=lambda item: item[1])]
