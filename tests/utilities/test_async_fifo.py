...
async def test_async_fifo_reset(dut: Asyncfifo):
    ...
    assert dut.data_out.value == 0, f"Initial data_out should be 0. Actual: {dut.data_out.value}"
    
    # Assert initial internal state
    assert dut.read_ptr.value == 0, f"Initial read_ptr should be 0. Actual: {dut.read_ptr.value}"
    assert dut.write_ptr.value == 0, f"Initial write_ptr should be 0. Actual: {dut.write_ptr.value}"
    assert dut.read_ptr_synced.value == 0, f"Initial read_ptr_synced should be 0. Actual: {dut.read_ptr_synced.value}"
    assert dut.write_ptr_synced.value == 0, f"Initial write_ptr_synced should be 0. Actual: {dut.write_ptr_synced.value}"
...
async def test_async_fifo_write_to_full_and_read_to_empty(dut: Asyncfifo):
    ...
    assert dut.full.value == 1, f"Full should be 1 after writing {write_count} times. Actual: {dut.full.value}"
    ...
    assert dut.write_ptr.value == dut.read_ptr.value, f"write_ptr and read_ptr should be equal after reading all data. write_ptr: {dut.write_ptr.value}, read_ptr: {dut.read_ptr.value}"
    assert dut.empty.value == 1, f"Empty should be 1 after reading all data. Actual: {dut.empty.value}"
...
async def test_async_fifo_write_read_different_clock(dut: Asyncfifo, read_clock_period: int, write_clock_period: int):
    dut._log.info(f"read_clock_period={read_clock_period}, write_clock_period={write_clock_period}")
    ...
    async def read_data():
        for i, expected_value in enumerate(data):
            ...
            assert actual_value == expected_value, f"Data mismatch at read {i}. Actual: {actual_value}, Expected: {expected_value}"
