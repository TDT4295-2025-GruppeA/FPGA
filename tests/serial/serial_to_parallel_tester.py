...
async def run_testdata(dut: Serialtoparallel, test_data: list[int], input_size: int, output_size: int, break_index: int | None = None):
    ...
    enable_break = break_index is not None
    if not enable_break:
        break_index = 0

    # Split each element into two ranges. This way, we can test having a
    # break between two elements.
    range1 = list(range(element_count))[:break_index]
...
async def tester_noncontinuous(
    dut: Serialtoparallel,
    test_data: list[int],
    input_size: int,
    output_size: int,
    ...
    break_index = len(test_data) // 2
    await run_testdata(dut, test_data, input_size, output_size, break_index)
    await run_testdata(dut, test_data, input_size, output_size, 0)
